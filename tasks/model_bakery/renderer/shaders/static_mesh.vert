...
void main(void)
{
  const vec4 wNorm = vec4(decode_normal_baked(floatBitsToUint(vPosNorm.w)), 0.0f);
  // const vec4 wNorm = vec4(decode_normal(floatBitsToUint(vPosNorm.w)),     0.0f);
  const vec4 wTang = vec4(decode_normal_baked(floatBitsToUint(vTexCoordAndTang.z)), 0.0f);
